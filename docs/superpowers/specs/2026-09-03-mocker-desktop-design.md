...
日期：2026-09-03  
状态：待实现  
方案：A — Rust + GPUI 桌面端，进程内 mock HTTP 服务
...
主用户：前端。日常路径是「建项目 → 选本机模型来源 → 粘贴导入 → 启动 → 改 baseURL → 点场景 → 看日志」。
...
- UI：GPUI + [gpui-component](https://github.com/longbridge/gpui-component)（表格、输入、列表、Dock）
...
```
┌──────────────────────────────────────────────────────────┐
│  项目切换    端口 7788  [启动/停止]    项目设置    导入     │
├─────────────┬────────────────────────────────────────────┤
│ 接口列表     │  当前接口                                   │
│ 搜索        │  路径 / 方法 / 名称 / 启用                    │
│ POST /path  │  当前场景：成功 | 空数据 | 参数错误 | 业务失败  │
│             │  入参字段表 | 响应字段树 + 该场景 JSON         │
├─────────────┴────────────────────────────────────────────┤
│ 请求日志：时间 方法 路径 状态 场景 耗时     [清空]          │
└──────────────────────────────────────────────────────────┘
```

- **顶栏**：项目切换、端口、启动/停止、项目设置、导入。全局模型来源放在应用设置（独立对话框），不挤在顶栏。
- **项目设置**：名称、端口、成功码、失败码、默认请求头表。
- **左侧列表**：扁平、可搜索。每行：方法、路径、当前场景标记、启用。支持新建、删除。不做模块树。
- **中间**：基本信息 + 场景选项 + 入参两表（Header / Body：英文名、中文名、类型、必填、说明/枚举）+ 响应（左树右 JSON）。提供「按字段重新生成该场景」「用 AI 重新生成成功数据」。
- **导入弹层**：文本框 → 解析（转圈、可取消进行中的模型请求）→ 预览表 → 写入当前项目。取消后不写库。
- **日志**：当前项目、最新在上、点行展开头/体/响应；按路径关键字过滤；清空。默认矮、可拉高。
- **空状态**：无项目则先建项目；无接口则主 CTA 为粘贴导入，其次手工新建。
- 第一版不做：多窗口、主题市场、客户端内试发请求。
...
5. GPUI 主窗口：项目、列表、启停、场景、JSON、日志
6. 字段树与 JSON 双向编辑、导入预览弹层、应用设置
